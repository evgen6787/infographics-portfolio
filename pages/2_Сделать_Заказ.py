...
# Функция бесплатной отправки лога заказа вам в Telegram
...
# Автопоиск баннера для левого меню
banner_file = None
...
# Инициализация памяти чата и шагов сценария
...
# Вывод сообщений на экран сайта
for message in st.session_state.messages:
    with st.chat_message(message["role"]): 
        st.write(message["content"])

# Логика интерактивного ассистента без внешних ИИ-зависимостей
if user_input := st.chat_input("Напишите ваш ответ здесь..."):
    ...
    # Проверка на глупые вопросы или флуд (Умный фильтр)
    ...
    # Если клиент пишет не по теме, бот мягко возвращает его назад
            ...
            with st.spinner("Бот пишет ответ..."):
                ...
                if st.session_state.step == 1:
                    ...
                    if "@" not in user_input and not has_digits:
                        ...
                        # Формируем и отправляем отчет вам в Telegram
                        full_order_log = f"""
🔔 **НОВЫЙ БРИФ С САЙТА!**
...
📝 **Преимущества (ТЗ):** {st.session_state.order_data.get('utp')}
                        ...
                        st.balloons()  # Включаем праздничные шарики
                else:
...
# Вывод кнопки перехода в ТГ-бота на финальном шаге
