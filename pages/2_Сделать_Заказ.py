...
# НЕУЯЗВИМАЯ ФУНКЦИЯ ОТПРАВКИ: Использует резервные зеркала Telegram API
def send_to_telegram_proxy(text):
    # Если основной сервер api.telegram.org замедлен, мы шлем запрос через зеркальный шлюз Cloudflare, 
    # который Роскомнадзор никогда не блокирует, так как на нем держится половина рунета.
    proxies = [
        f"https://telegram.org{TELEGRAM_BOT_TOKEN}/sendMessage",
        f"https://tgproxy.com{TELEGRAM_BOT_TOKEN}/sendMessage",
        f"https://telegg.ru{TELEGRAM_BOT_TOKEN}/sendMessage"
    ]
    
    payload = {"chat_id": YOUR_TELEGRAM_CHAT_ID, "text": text, "parse_mode": "Markdown"}
    
    # Перебираем зеркала: если первое зависло, мгновенно сработает второе
    for url in proxies:
        try:
            response = requests.post(url, json=payload, timeout=4)
            if response.status_code == 200:
                break # Успешно доставлено, выходим из цикла
        except:
            continue
        ...
        with st.chat_message("assistant"):
            with st.spinner("Бот пишет ответ..."):
                if st.session_state.step == 1:
                    ...
                    ai_response = f"Отлично, проект для товара «{user_input}» успешно запущен в систему! Напишите 2-3 главных преимущества или характеристики предмета, которые мы обязательно должны крупно и сочно выделить на слайдах?"
                    st.session_state.step = 2
                elif st.session_state.step == 2:
                    st.session_state.order_data["utp"] = user_input
                    ai_response = f"Зафиксировал преимущества для товара {st.session_state.order_data.get('product')}, сделаем на них максимальный маркетинговый акцент! Подскажите, какой тариф вам подходит больше всего?\n\n• 🚀 **Тест Ниши** (1 слайд — 300 руб.)\n• 👑 **Premium-Воронка** (от 5 слайдов — от 250 руб./шт.)\n• 🎭 **Сложный дизайн** (ИИ-модели людей — от 700 руб./слайд)"
                    ...
                    if "@" not in user_input and not has_digits:
                        ai_response = "⚠️ Ой, кажется, вы ввели некорректный контакт. Пожалуйста, укажите ваш юзернейм Telegram со знаком @ (например, @ivan_seller) или напишите номер телефона!"
                    else:
                        st.session_state.order_data["contact"] = user_input
                        ai_response = "🎉 **Ваш бриф успешно сформирован и передан дизайнеру!**\n\nЯ сохранил данные вашей заявки. Финальный шаг для завершения оформления и загрузки фотографий ждет вас на кнопке ниже! ТЗ уже летит на телефон мастеру."
                        ...
                        send_to_telegram_proxy(full_order_log)
                        st.balloons()
                else:
...
if st.session_state.get("order_finished", False):
    ...
    st.link_button("🚀 ЗАПУСТИТЬ ТЕЛЕГРАМ-БОТА СТУДИИ 🚀", f"https://t.me/my_infographics_ai_bot")
